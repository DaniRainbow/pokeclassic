...
These instructions explain how to set up the tools required to build **pokeclassic**, which assembles the source files into a ROM.
...
## Windows 10 (WSL1)
WSL1 is the preferred terminal to build **pokeclassic**. The following instructions will explain how to install WSL1 (referred to interchangeably as WSL).
...
- Otherwise, **open WSL** and go to [Choosing where to store pokeclassic (WSL1)](#Choosing-where-to-store-pokeclassic-WSL1).
...
4. Certain packages are required to build pokeclassic. Install these packages by running the following command:
...
### Choosing where to store pokeclassic (WSL1)
WSL has its own file system that's not natively accessible from Windows, but Windows files *are* accessible from WSL. So you're going to want to store pokeclassic within Windows.

For example, say you want to store pokeclassic (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps**. First, ensure that the folder already exists. Then, enter this command to **change directory** to said folder, where *\<user>* is your **Windows** username:
...
- Otherwise, **open msys2** and go to [Choosing where to store pokeclassic (msys2)](#choosing-where-to-store-pokeclassic-msys2).
...
2. Certain packages are required to build pokeclassic. Install these by running the following command:
...
### Choosing where to store pokeclassic (msys2)
At this point, you can choose a folder to store pokeclassic into. If you're okay with storing pokeclassic in the user profile folder, then proceed to [Installation](#installation). Otherwise, you'll need to account for where pokeclassic is stored when changing directory to the pokeclassic folder.

For example, if you want to store pokeclassic (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps** (where *\<user>* is your **Windows** username), enter this command:
...
2.
    ...
    - Otherwise, **open Cygwin** and go to [Choosing where to store pokeclassic (Cygwin)](#choosing-where-to-store-pokeclassic-cygwin)
...
### Choosing where to store pokeclassic (Cygwin)

Cygwin has its own file system that's within Windows, at **C:\cygwin64\home\\_\<user>_**. If you don't want to store pokeclassic there, you'll need to account for where pokeclassic is stored when **changing directory** to the pokeclassic folder.

For example, if you want to store pokeclassic (and agbcc) in **C:\Users\\_\<user>_\Desktop\decomps**, enter this command, where *\<user>* is your **Windows** username:
```bash
cd c:/Users/<user>/Desktop/decomps
```
Note that the directory **must exist** in Windows. If you want to store pokeclassic in a dedicated folder that doesn't exist (e.g. the example provided above), then create the folder (e.g. using Windows Explorer) before executing the `cd` command.
...
2.  - If libpng is **not installed**, then go to [Installing libpng (macOS)](#installing-libpng-macos).
    ...
    - Otherwise, **open the Terminal** and go to [Choosing where to store pokeclassic (macOS)](#choosing-where-to-store-pokeclassic-macos)
...
3. Run the following command to install libpng.
    ...
    Continue to [Installing devkitARM (macOS)](#installing-devkitarm-macos) if **devkitARM is not installed**, otherwise, go to [Choosing where to store pokeclassic (macOS)](#choosing-where-to-store-pokeclassic-macos).
...
### Choosing where to store pokeclassic (macOS)
At this point, you can choose a folder to store pokeclassic into. If you're okay with storing pokeclassic in the user folder, then proceed to [Installation](#installation). Otherwise, you'll need to account for where pokeclassic is stored when changing directory to the pokeclassic folder.

For example, if you want to store pokeclassic (and agbcc) in **~/Desktop/decomps**, enter this command to **change directory** to the desired folder:
```bash
cd Desktop/decomps
```
Note that the directory **must exist** in the folder system. If you want to store pokeclassic in a dedicated folder that doesn't exist (e.g. the example provided above), then create the folder (e.g. using Finder) before executing the `cd` command.
...
sudo apt install build-essential binutils-arm-none-eabi git libpng-dev
```
Then proceed to [Choosing where to store pokeclassic (Linux)](#choosing-where-to-store-pokeclassic-linux).
<details>
...
### Choosing where to store pokeclassic (Linux)
At this point, you can choose a folder to store pokeclassic (and agbcc) into. If so, you'll have to account for the modified folder path when changing directory to the pokeclassic folder.
...
>   Consider adding an exception for the `pokeclassic` and/or `decomps` folder in Windows Security using
...
1. If pokeclassic is not already downloaded (some users may prefer to download pokeclassic via a git client like GitHub Desktop), run:

    ```bash
    git clone https://github.com/DaniRainbow/pokeclassic.git
    ```
    ...
    >   cd <folder where pokeclassic is to be stored>
    >   ```
    >   Where *\<folder where pokeclassic is to be stored>* is the path of the folder [where you chose to store pokeclassic](#Choosing-where-to-store-pokeclassic-WSL1). Then run the `git clone` command again.
    </details>

2. Install agbcc into pokeclassic. The commands to run depend on certain conditions. **You should only follow one of the listed instructions**:
- If agbcc has **not been built before** in the folder where you chose to store pokeclassic, run the following commands to build and install it into pokeclassic:

    ```bash
    git clone https://github.com/pret/agbcc
    cd agbcc
    ./build.sh
    ./install.sh ../pokeclassic
    ```

- **Otherwise**, if agbcc has been built before (e.g. if the git clone above fails), but was **last built on a different terminal** than the one currently used (only relevant to Windows, e.g. switching from msys2 to WSL1), then run the following commands to build and install it into pokeclassic:
...
- **Otherwise**, if agbcc has been built before on the same terminal, run the following commands to install agbcc into pokeclassic:
    ...
    <details>
        ...
        > If building agbcc or pokeclassic results in an error, try deleting the agbcc folder and re-installing agbcc as if it has not been built before.
    </details>

3. Once agbcc is installed, change directory back to the base directory where pokeclassic and agbcc are stored:

    ```bash
    cd ..
    ```

Now you're ready to [build **pokeclassic**](#build-pokeclassic)
## Build pokeclassic
If you aren't in the pokeclassic directory already, then **change directory** to the pokeclassic folder:
```bash
cd pokeclassic
```
To build **pokeclassic.gba** (Note: to speed up builds, see [Parallel builds](#parallel-builds)):
```bash
make
```
If it has built successfully you will have the output file **pokeclassic.gba** in your project folder. Congratulations! You've reached the end of this tutorial.
<details>
...
* [porymap](https://github.com/huderlem/porymap) for viewing and editing maps
	**Please note that you will need to use Porymap 4.5.0 with PokeClassic. Anything newer may cause issues as the newer tool wasn't designed to work with older versions of PRET's pokeclassic**
